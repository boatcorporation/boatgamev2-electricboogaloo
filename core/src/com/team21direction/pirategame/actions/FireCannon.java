...
public class FireCannon extends Action {
  ...
  /**
   * Shoot the college's cannons at the player.
   *
   * @param delta the time since the last frame.
   * @return whether the college is defeated.
   */
  @Override
  public boolean act(float delta) {
    ...
    if (timeSinceLastCannon >= 2.0f + Math.random()) {
      college.screen.fireCannon(
          college,
          new Vector2(
                  college.screen.player.getX() - college.getX(),
                  college.screen.player.getY() - college.getY())
              .nor()
              .scl(3.0f));
